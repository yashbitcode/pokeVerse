...
const QuizComp = () => {
    ...
    return (
        <div className="mt-[2rem] w-full max-w-[800px] mx-auto">
            ...
            <div className="mt-[10px] text-xl">{`${currentQ + 1}. ${quizQuestions[0].question}`}</div>

            <QuizOptions options={quizQuestions[currentQ].options} answer={quizQuestions[currentQ].answer} />

            <div className="text-[0.9rem] mx-auto w-fit mt-[1rem]">{`${currentQ + 1} of ${quizQuestions.length + 1} Questions`}</div>
        </div>
    );
};
